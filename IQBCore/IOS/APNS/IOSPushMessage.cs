...
namespace IQBCore.IOS.APNS
{
    public class IOSPushMessage
    {
        ...
        public IOSPushMessage(IOSPushType pushType, string certFilePath, string certPwd)
        {
            string _host = "";
            int _port = 0;

            if (pushType == IOSPushType.Development)
            {
        ...
        public void Push(IOSPushSetting ps)
        {
            string payloadStr = ps.PushPayload();
            string deviceToken = ps.deviceToken;
